...
def handle_query(df: pd.DataFrame, query: str) -> str:
    ...
    # === BAGIAN YANG DIREVISI MULAI DI SINI ===
    if matches.empty:
        if re.search(r"\bvoucher\b", query, re.IGNORECASE):
            model = genai.GenerativeModel("models/gemini-flash-latest")
            check_prompt = f"""
            Pertanyaan kasir: "{query}"
            Jawab singkat ya, hanya 1 kalimat.
            Cek apakah voucher ini bisa digunakan di AZKO (Ace/INFORMA/Toy Kingdom).
            Kalau voucher itu tidak bisa, jawab: 'voucher tersebut belum bisa digunakan di AZKO'.
            Kalau tidak yakin, jawab: 'saya belum dapat info pastinya'.
            """
            try:
                response = model.generate_content(check_prompt)
                check_result = response.text.strip() if response and response.text else ""
            except Exception:
                check_result = "saya belum dapat info pastinya"

            return (
                f"Hmm, sepertinya promo atau voucher itu belum ada di data aku nih.\n\n"
                f"Berdasarkan pengecekan cepat, {check_result}.\n\n"
                f"Untuk kepastian lebih lanjut, silakan cek email dari *Partnership*, *PNA*, "
                f"atau tanyakan langsung ke *Finrep Area* kamu ya 😊"
            )
        else:
            return (
                "Hmm, sepertinya promo yang kamu maksud belum ada di data aku nih.\n\n"
                "Untuk kepastian lebih lanjut, silakan cek email dari *Partnership*, *PNA*, "
                "atau tanya ke *Finrep Area* kamu ya 😊"
            )
    # === REVISI SELESAI ===
